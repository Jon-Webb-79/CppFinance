...
//  RNG
//
//  Created by Jon Webb on 11/14/14.
//  Copyright (c) 2014 Jon Webb. All rights reserved.
//
// This algorithm and code is outlined in;
// - F. Brown, "MCNP5 Random Number Generator," LA-UR-07-7961,
//   LA-UR-07-7961, 2007
//
//========================================================================
// C version of MCNP5 routines for random number generation
//
//  - These routines require the use of 'unsigned long lon' integers,
...
//     routines will have types of U8 or I4.
...
//  * 01-29-2005  - J Sweezy, modify to use mcnp modules prior to automitc
//                  io unit numbers.
//  * 12-02-2005  - F Brown, mods for consistency with C version
//========================================================================


#ifndef __RNG__Random_Number_Generator__
#define __RNG__Random_Number_Generator__
...
//--------------------------------------------------
// Kinds of LONG INTEGERS (64-bit) & REAL*8 (64-bit)
//--------------------------------------------------
...
void   RN_init_problem( int* new_standard_gen, ULONG* new_seed,
                        ULONG* new_stride, ULONG* new_part1,
                        int* print_info);
...
ULONG   RN_skip_ahead( ULONG* s, LONG* n ) {
    ...
    ULONG  gen=1,  g=RN_MULT, inc=0, c=RN_ADD, rn;
...
void RN_test_basic( int* new_gen ) {
    ...
    for( i=0; i<5;      i++ ) { s += rang(); seeds[i] = RN_query(strcpy(seed,"seed")); }
    for( i=5; i<123455; i++ ) { s += rang(); }
    for( i=5; i<10;     i++ ) { s += rang(); seeds[i] = RN_query(strcpy(seed,"seed")); }
...
#endif /* defined(__RNG__Random_Number_Generator__) */
